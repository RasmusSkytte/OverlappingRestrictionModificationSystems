...
from helpers import default_parameters, make_dirs, get_path, set_rc_params, pickle_read, compute_gamma_and_omega, add_figure_labels
...
fig_path  = get_path()
make_dirs(os.path.join(fig_path, 'Figure_S15'))
...
for simulation_id, (xlim, ylim) in tqdm(enumerate(zip([0.97, 0.94], [-10.0, -10.0])), total=2) :
    ...
    # Load the data
    lname = os.path.join(data_path, 'FigS15_S16_S17', f'iterations_1e{np.log10(iterations):.0f}_T_1e{np.log10(T):.1f}_C_1e{np.log10(C):.1f}', f'RM_{len(RMs)}_seed_0.pkl')
...
fig.savefig(os.path.join(fig_path, 'Figure_S15', 'figS15.png'), bbox_inches = 'tight')