...
A_ij, s, data_taxonomy, _, species_id_to_name = load_sequence_data(grouping = 'species')


# Count genera
species = len(list(iter_sequence_data(A_ij, s, data_taxonomy, grouping = 'species', tqdm_disable=True)))
...
plot_id = 0
ax_id = 0
for _, A_ij_s, Bs, species in iter_sequence_data(A_ij, s, data_taxonomy, grouping = 'species') :
    ...
    if plot_id % n_plots == 0 or (panel_id == 0 and plot_id == n_cols * n_rows_first):
        if plot_id > 0 :
            ...
            fig.savefig(os.path.join(fig_path, f'figS7_{panel_id}.png'), bbox_inches = 'tight')
    ...
    # Add the plots
    plot_bipartite_network(Bs, A_ij_s, A_RMs_s, species, scaling=0.5, title_scale=1.1, ax=axes[ax_id])
